...
func verifyBlockCopy(d *disk.Disk, r partitionResizeTarget, expectedSize int64) error {
	// open both partitions for reading
	origPart, err := d.GetPartition(r.original.number)
	if err != nil {
		return err
	}
	targetPart, err := d.GetPartition(r.target.number)
	if err != nil {
		return err
	}
